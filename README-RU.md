...
Для установки vcpkg нужно следовать инструкциям тут: https://github.com/Microsoft/vcpkg
...
Далее нужно задать переменную среды VCPKG_ROOT указывающую на директорию в которую установлен vcpkg.
...
В директории webtest/webui лежит исходный код интерфейсной части приложения для интерактивного
...
Каждый том состоит из 3-х файлов с расширениями .phkvsmain .phkvsstm и .phkvsbig 
...
слишком больших для inplace хранения в основном файле.
...
|Head first free node    | uint64_t   |          | Смещение первой свободного головного узла skip-list-а. 0 если нет свободных головных узлов.|
|List first free node    | uint64_t   |          | Смещение первой свободного узла skip-list-а. 0 если нет свободных узлов.                   |

Сразу за заголовком идёт головной узел корневой директории.
Head first free node и List first free node - свободные узлы организованы в single linked list. Смещение следующего
элемента лежит в первых 64 битах. Список заканчивается 0-м.

Головной узел:

|Поле                    | Тип/Размер | Описание                                                                               |
|------------------------|------------|----------------------------------------------------------------------------------------|
|nexts level             | uint8_t    | Количество уровней в массиве указателей на следующий узел                              |
|next offset/stm offset  | uint64_t   | Смещение следующего узла если nexts level == 1 или смещение в phkvsstm файле если >1   |

Каждый skip-list всегда начинается с головного узла без значений.

Узел с данными block-skip-list-а:

|Поле                    | Тип/Размер | Описание                                                                               |
|------------------------|------------|----------------------------------------------------------------------------------------|
|nexts level             | uint8_t    | Количество уровней в массиве указателей на следующущий узел                            |
|next offset/stm offset  | uint64_t   | Смещение следующего узла если nexts level == 1 или смещение в phkvsstm файле если >1   |
|used entries count      | uint8_t    | Количество заполненных элементов с данными                                             |
|entry array             | Entry[16]  | Элементы с данными                                                                     |

Узел имеет фиксированный размер. Не занятые ячейки с данными заполненны нулями.
...
*ВАЖНО* - в файле НЕ хранится размер выделенного блока. Это ответственность пользователя
...
то переходим в этот узел. Иначе опускаемся на уровень ниже и повторяем. Если уже на 0-м уровне, прекращаем поиск.
